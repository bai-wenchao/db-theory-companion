...
def main() -> int:
    ...
    w("")
    w("% Wide-margin convention (user spec): entries live in the TEXT column")
    w("% only, at the book's normal measure -- the margin column stays free on")
    w("% every page (odd: right margin, even: left margin), exactly like the")
    w("% body chapters. The text measure is identical on both parities, so NO")
    w("% parity machinery (fullwidth/adjustwidth*, aux page probes, leftskip")
    w("% tricks) is needed at all -- those earlier schemes were the source of")
    w("% the half-off-paper and margin-filling bugs. Each entry is ONE")
    w("% unbreakable paragraph (\\interlinepenalty 10000): a line never splits")
    w("% across pages, and a domain header (\\idxdom joins header + first")
    w("% entry with \\\\*) can never be orphaned. Spacing: \\parskip=0 so a")
    w("% domain header has NO breakable glue before its first entry; the")
    w("% breakable separator between entries is \\idxsep instead, and")
    w("% \\idxfirstline (no separator) follows headers and section heads.")
    w("\\setlength{\\parskip}{0pt}")
    w("\\newcommand*{\\idxentry}[1]{%")
    w("  \\par")
    w("  \\begingroup")
    w("    \\interlinepenalty=10000 %")
    w("    % narrow measure + unbreakable \\mbox{Thm.~n.m} tails + a NARROWER")
    w("    % hangindent continuation line can exhaust the global 3em emergency")
    w("    % stretch (one 6.8pt overfull observed); give entries more slack")
    w("    \\emergencystretch=6em\\relax")
    w("    \\noindent #1\\par")
    w("  \\endgroup}")
