...
\item (Chain rule.) $D(p(x)P(z|x) \,\|\, p(x)Q(z|x)) = \sum_x p(x)\,
D(P(\cdot|x) \,\|\, Q(\cdot|x))$: with the same mixing distribution on both sides,
the joint divergence is the average of the conditional divergences. Consequently
...
\paragraph{Problem.}
Queries are \emph{disjunctive datalog rules} (DDRs) $Q(Z) :- \bigvee_{R(X) \in
\Sigma_{\mathrm{in}}} R(X)$: an output \emph{model} assigns tuples to the atoms
