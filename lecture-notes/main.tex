...
% ------------------------------------------------------------------
% A Theoretical Companion to Database Research
...
% Build: ./build.sh [main|chapter <name>|clean]  (writes buildstamp.tex, wipes aux, reruns
...
% paragraph-level relief for justified text: TeX's emergency pass can then loosen
% a problem line instead of overrunning the measure (overfull polish pass).
...
\fancyhf{}
\fancyhead[LE]{\small\thepage\enspace\itshape A Theoretical Companion to Database Research}
...
\title{A Theoretical Companion to Database Research}
% optional arg = plain-text author for PDF metadata (tufte-common.def);
% stacked display: human author, then the model credit one size down
\author[Wenchao Bai and GLM5.3 (harnessed by Claude Code)]{Wenchao Bai\\[0.6ex]{\large GLM5.3 (harnessed by Claude Code)}}
\date{September 2026}

% book version, printed on the verso-of-title page (bump at milestones):
\newcommand{\bookversion}{1.0}
\newcommand{\bookstatus}{First complete edition}
...
\null\vfill
\begin{center}
{\Large\itshape A Theoretical Companion to Database Research}\\[3ex]
...
Commons Attribution 4.0 International Public License (\textsc{cc\,by\,4.0}):
you may share and adapt it for any purpose with attribution --- see
\url{https://creativecommons.org/licenses/by/4.0/} for details.\\[2ex]
% badge: vector; build.sh converts the tracked cc-by.svg to cc-by.pdf
\href{https://creativecommons.org/licenses/by/4.0/}{\includegraphics{cc-by}}\\[2ex]
Text available on
\url{https://github.com/bai-wenchao/db-theory-companion} --- please post any
issues there --- thank you!\\[2ex]
...
% (52 papers) leads; concentration (39) is Chapter 2 -- counts from the
% ch0 map table (tagged papers per tool)
\include{chapters/ch0}
\include{chapters/reduction}
\include{chapters/concentration-ineq}
\include{chapters/induction}
\include{chapters/estimation-theory}
\include{chapters/adversarial-construction}
\include{chapters/dp-composition}
\include{chapters/exchange-greedy}
\include{chapters/spectral-matrix}
\include{chapters/amortized-potential}
\include{chapters/coresets-rnla}
\include{chapters/information-theory}
\include{chapters/communication-complexity}
\include{chapters/online-decisions}

% back matter: script-generated (\ref-based, no makeindex); regenerate with
% python3 scripts/11_make_index.py
